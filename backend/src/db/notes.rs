...
pub async fn get_note(
    ...
    let row: Option<(String,)> =
        sqlx::query_as(r#"SELECT content FROM notes WHERE group_id = $1 AND name = $2"#)
            .bind(group_id)
            .bind(name)
            .fetch_optional(pool)
            .await?;
...
pub async fn list_notes(pool: &PgPool, group_id: i64) -> Result<Vec<String>, sqlx::Error> {
    let rows: Vec<(String,)> =
        sqlx::query_as(r#"SELECT name FROM notes WHERE group_id = $1 ORDER BY name"#)
            .bind(group_id)
            .fetch_all(pool)
            .await?;
...
pub async fn delete_note(pool: &PgPool, group_id: i64, name: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query(r#"DELETE FROM notes WHERE group_id = $1 AND name = $2"#)
        .bind(group_id)
        .bind(name)
        .execute(pool)
        .await?;
