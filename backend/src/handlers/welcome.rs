            ...
            Ok(Some(settings)) => {
                if let Some(ref welcome) = settings.welcome_message {
                    let _ = bot
                        .send_message(
                            msg.chat.id,
                            format!("Current welcome message:\n{}", welcome),
                        )
                        .await;
                } else {
        ...
        match crate::db::welcome::get_welcome_settings(client, chat_id).await {
            Ok(Some(settings)) => {
                if let Some(ref dm) = settings.welcome_dm_message {
                    let _ = bot
                        .send_message(msg.chat.id, format!("Current welcome DM message:\n{}", dm))
                        .await;
                } else {
                    let _ = bot
                        .send_message(
                            msg.chat.id,
                            "No welcome DM message set. Send /setwelcomedm <message> to set one.",
                        )
                        .await;
                }
            }
            ...
            Ok(Some(settings)) => {
                if let Some(ref farewell) = settings.farewell_message {
                    let _ = bot
                        .send_message(
                            msg.chat.id,
                            format!("Current farewell message:\n{}", farewell),
                        )
                        .await;
                } else {
                    let _ = bot
                        .send_message(
                            msg.chat.id,
                            "No farewell message set. Send /setfarewell <message> to set one.",
                        )
                        .await;
                }
            }
