...
use crate::db::games::{get_bounty, claim_daily_bounty};
...
pub async fn handle_daily(bot: Bot, msg: Message, client: &Client) -> Result<(), String> {
    ...
    match claim_daily_bounty(client, user_id as i64).await {
        ...
        Ok(Err(err_msg)) => {
            let _ = bot.send_message(msg.chat.id, crate::utils::escape_md_v2(&err_msg))
                .await;
        },
        Err(e) => {
            tracing::error!("Error claiming daily bounty: {}", e);
            let _ = bot.send_message(msg.chat.id, "I'm sorry, I couldn't process your daily bounty at this time.")
                .await;
        }
    }
