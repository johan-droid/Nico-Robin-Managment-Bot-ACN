use crate::db::games::{
    create_crew, get_crew_bounty, get_crew_by_member, invite_to_crew, join_crew, leave_crew,
};
...
pub async fn handle_crew(bot: Bot, msg: Message, client: &Client) -> Result<(), String> {
    ...
    match action.as_str() {
        "create" => {
            if parts.len() < 3 {
                let _ = bot
                    .send_message(msg.chat.id, "Please specify a crew name!")
                    .await;
                return Ok(());
            }
            ...
            if let Ok(Some(_)) = get_crew_by_member(client, user_id as i64).await {
                let _ = bot
                    .send_message(msg.chat.id, "You are already in a crew!")
                    .await;
                return Ok(());
            }
                ...
                Ok(id) => {
                    let _ = bot
                        .send_message(
                            msg.chat.id,
                            format!(
                                "Congratulations, Captain! The *{}* has been formed.\nCrew ID: {}",
                                name, id
                            ),
                        )
                        .await;
            ...
            if let Some(reply) = &msg.reply_to_message {
                if let Some(target_user) = reply.from() {
                    if target_user.id == user_id {
                        let _ = bot
                            .send_message(msg.chat.id, "You cannot invite yourself!")
                            .await;
                        return Ok(());
                    }
        ...
        "leave" => match leave_crew(client, user_id as i64).await {
            Ok(_) => {
                let _ = bot
                    .send_message(
                        msg.chat.id,
                        "You have successfully left the crew. Safe travels!",
                    )
                    .await;
            }
            Err(e) => {
        ...
        _ => {
            let _ = bot
                .send_message(
                    msg.chat.id,
                    "Unknown crew action. Valid actions: create, info, invite, join, leave.",
                )
                .await;
        }
    }
