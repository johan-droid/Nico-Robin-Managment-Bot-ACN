...
pub async fn handle_gban(bot: Bot, msg: Message, client: &Client) -> Result<(), String> {
    ...
    let reason = parts.get(1).map(|s| s.trim()).filter(|s| !s.is_empty()).unwrap_or("No reason provided");
    ...
    let (target_id, target_name) = match crate::auth::extract_target_user(&mock) {
        Some((0, name)) => {
            match crate::auth::resolve_username(&bot, msg.chat.id, &name).await {
                Some(v) => v,
                None => {
                    ...
                    match client.query_one("SELECT user_id, first_name FROM username_cache WHERE username = $1", &[&clean_uname]).await {
                        Ok(row) => (row.get::<_, i64>(0), crate::crypto::try_decrypt(&row.get::<_, String>(1))),
                        Err(_) => {
                            send_text(&bot, msg.chat.id, "User not found. Reply to their message or use their numeric ID.").await;
                            return Ok(());
                        }
                    }
                }
    ...
    if let Err(e) = crate::db::gbans::add_gban(client, target_id, &target_name, reason, msg.from().map(|u| u.id as i64).unwrap_or(0)).await {
        send_text(&bot, msg.chat.id, &format!("Error: {}", escape_md_v2(&e))).await;
        return Ok(());
    }

    // Ban the user from every group the bot knows about.
    let groups = crate::db::groups::list_groups(client).await.unwrap_or_default();
...
pub async fn handle_ungban(bot: Bot, msg: Message, client: &Client) -> Result<(), String> {
    ...
    match crate::db::gbans::remove_gban(client, target_id).await {
        Ok(true) => {
            send_text(&bot, msg.chat.id, &format!("Removed {} from the global ban list.", escape_md_v2(&target_name))).await;
            let executor = msg.from().map(|u| u.first_name.as_str()).unwrap_or("Admin");
            log_mod_action(
                &bot,
                crate::config::Settings::global(),
