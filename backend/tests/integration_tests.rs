...
use nico_robin_bot::utils::error::{report_failure, sanitize_secrets, BotError};
...
fn test_sanitize_secrets_and_sentry_scrubber() {
    ...
    std::env::set_var(
        "DATABASE_URL",
        "postgresql://robin:password@localhost/robin_db",
    );
    ...
    let sanitized_breadcrumb = sanitized_event.breadcrumbs.values[0]
        .message
        .as_ref()
        .unwrap();
