...
async def management_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    response = (
        ...
        "Keep it simple:\n"
        "• `/ban`, `/unban`, `/kick` - member control\n"
        "• `/mute`, `/unmute` - silence or restore chat access\n"
        "• `/warn`, `/warns`, `/resetwarn` - warning flow\n"
        "• `/del`, `/purge`, `/pin` - message cleanup\n"
        "• `/rules`, `/setrules` - group rules\n"
        "• `/stats`, `/id`, `/whois` - quick group info\n\n"
        "If you want more commands later, I can expand this guide again."
    )
